...
function run_emqx() {
    ...
    docker run -d -t --restart=always --name "$name" \
      --net "$NET" \
      -e EMQX_LOG__CONSOLE__LEVEL="${LOG_LEVEL:-debug}" \
      -e EMQX_NODE_NAME="emqx@$node" \
      -e EMQX_NODE_COOKIE="$COOKIE" \
      -e EMQX_NODE__ROLE="$role" \
      -e EMQX_LICENSE__KEY="${LICENSE_KEY}" \
      -e EMQX_PLUGINS__STATES="[{enable = true, name_vsn = \"emqx_ecq-${PLUGIN_VSN}\"}]" \
...
validate_plugin() {
    local container="$1"
    local plugin_info
    plugin_info=$(docker exec -t "$container" emqx ctl plugins list | jq -r '.[] | select(.name == "emqx_ecq")')
    if [ -z "$plugin_info" ]; then
        echo "Error: Plugin emqx_ecq not found in node $container"
        exit 1
    fi
    local plugin_vsn
...
echo "Plugin emqx_ecq-${PLUGIN_VSN} is running in all nodes."

cmd() {
    docker exec node1.emqx.io emqx ctl ecq $@
}
...
docker exec node1.emqx.io emqx eval 'application:stop(emqx_telemetry)'
docker exec node2.emqx.io emqx eval 'application:stop(emqx_telemetry)'
docker exec node3.emqx.io emqx eval 'application:stop(emqx_telemetry)'
